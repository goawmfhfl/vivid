...
import { updateInvestmentTrendsForUser, type UserInvestmentTrendResult } from "../helpers";
import { CRON_CONCURRENCY } from "@/constants/cron";

export const dynamic = "force-dynamic";
export const maxDuration = 180;

type InvestmentTrendsBatchPayload = {
  userIds: string[];
};
...
function getConcurrency(value: string | null): number {
  const fallback = CRON_CONCURRENCY.USER_TRENDS_WEEKLY;
...
async function parseQstashPayload(
  request: NextRequest
): Promise<InvestmentTrendsBatchPayload> {
  const receiver = getQstashReceiver();
  const signature = request.headers.get("upstash-signature") || "";
  const body = await request.text();
  const verificationUrl = getSignatureVerificationUrl(request);

  const isValid = await receiver.verify({ signature, body, url: verificationUrl });
  if (!isValid) throw new Error("Invalid QStash signature");

  return JSON.parse(body) as InvestmentTrendsBatchPayload;
}

async function processWithConcurrency(
  userIds: string[],
  concurrency: number,
  handler: (userId: string) => Promise<UserInvestmentTrendResult>
): Promise<UserInvestmentTrendResult[]> {
  const results = new Array<UserInvestmentTrendResult>(userIds.length);
  let index = 0;
  ...
  const workers = Array.from({ length: Math.min(concurrency, userIds.length) }, async () => {
    while (true) {
      const current = index;
      index += 1;
      if (current >= userIds.length) break;
...
export async function POST(request: NextRequest) {
  try {
    let payload: InvestmentTrendsBatchPayload;

    if (isCronSecretAuthorized(request)) {
      payload = (await request.json()) as InvestmentTrendsBatchPayload;
    } else {
    ...
    const results = await processWithConcurrency(userIds, concurrency, async (userId) => {
      try {
        return await updateInvestmentTrendsForUser(supabase, userId);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error(`[update-investment-trends] user ${userId} failed:`, message);
        return { userId, status: "skipped" as const, reason: message };
      }
    });
    ...
    console.log("[cron] update-investment-trends batch processed:", {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error("Cron update-investment-trends batch error:", error);
