...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Daily Feedback By Range] userId: ${userId}, month: ${month}`);
    console.log(`[Daily Feedback By Range] dateRange: ${dateRange.start_date} ~ ${dateRange.end_date}`);
    ...
    if (error) {
      console.error(`[Daily Feedback By Range] Supabase error:`, error);
      throw new Error(`Failed to fetch daily feedbacks: ${error.message}`);
    }

    console.log(`[Daily Feedback By Range] Found ${data?.length || 0} daily feedbacks`);
