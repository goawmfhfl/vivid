...
**비전 진화 스토리 요약:**
- 핵심 비전: ${report.vision_evolution.core_visions.map(v => v.vision).slice(0, 3).join(", ")}

**일치도 분석 요약:**
- 주요 격차: ${report.alignment_analysis.gap_analysis.biggest_gaps.map(g => g.gap_description).slice(0, 1).join(", ") || "없음"}

**하루 패턴 요약:**
- 주요 패턴: ${report.daily_life_patterns.recurring_patterns.map(p => p.pattern).slice(0, 2).join(", ") || "없음"}
- 주요 강점: ${report.daily_life_patterns.evaluation_themes.strengths.map(s => s.theme).slice(0, 2).join(", ") || "없음"}
