...
import { decryptWeeklyVivid } from "@/lib/jsonb-encryption";

/**
 * 주간 비비드가 특정 월에 속한 일수를 계산
 * @param weekStart 주간 비비드 시작일 (YYYY-MM-DD)
 * @param weekEnd 주간 비비드 종료일 (YYYY-MM-DD)
 ...
 *   - 11월: 3일 (11/1, 11/2) → 제외 ❌
 */
export async function fetchWeeklyVividByMonth(
  supabase: SupabaseClient,
  userId: string,
  startDate: string,
  endDate: string
): Promise<WeeklyVivid[]> {
  // 1단계: 주간 비비드가 해당 월과 겹치는 경우 모두 조회
  ...
  // 2단계: 각 주간 비비드에 대해 해당 월에 속한 일수 계산
