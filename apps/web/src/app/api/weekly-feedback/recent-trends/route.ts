...
/**
 * GET 핸들러: 최근 4주간의 weekly-feedback trend 데이터 조회
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // 최근 4주간의 weekly-feedback 데이터 조회 (trend 컬럼만 선택)
    ...
    if (error) {
      throw new Error(`Failed to fetch weekly feedback trends: ${error.message}`);
    }
