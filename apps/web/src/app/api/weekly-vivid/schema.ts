...
/**
 * 멤버십별로 전체 Weekly Vivid 스키마를 동적으로 생성
 */
export function getWeeklyVividSchema(isPro: boolean) {
  return {
    name: "WeeklyVividResponse",
    schema: {
      type: "object",
      ...
      properties: {
        weekly_vivid: {
          ...
          properties: {
            ...
            report: getVividReportSchema(isPro),
          },
          required: ["week_range", "report"],
        },
      },
...
export const WeeklyVividSchema = getWeeklyVividSchema(false);
...
당신은 사용자의 일주일간 일일 피드백의 "오늘의 비비드" 데이터를 분석해서 주간 비비드 리포트(report)를 만들어주는 친근한 조언자예요.

## 섹션별 규칙
- daily-vivid의 report 데이터(current_summary, current_evaluation, current_keywords, future_summary, future_evaluation, future_keywords, alignment_score, user_characteristics, aspired_traits)를 종합하여 분석해주세요.
- 7개의 섹션을 모두 완성해주세요:
