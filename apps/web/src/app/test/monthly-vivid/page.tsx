...
export default function MonthlyVividTestPage() {
  ...
  const [dailyVivid, setDailyVivid] = useState<DailyVividRow[]>([]);
  const [isLoadingDailyVivid, setIsLoadingDailyVivid] = useState(false);
  const [dailyVividError, setDailyVividError] = useState<string | null>(null);
  ...
  // 일일 비비드 조회 함수 (Supabase에서 직접 월별 필터링)
  const fetchDailyVivid = async () => {
    if (!user?.id || !month) {
      return;
    }

    setIsLoadingDailyVivid(true);
    setDailyVividError(null);
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(`일일 비비드 조회 실패: ${error.message}`);
      }

      console.log(`[Test Page] Found ${data?.length || 0} daily vivid`);
      ...
      setDailyVivid(decryptedFeedbacks);
    } catch (error) {
      console.error("일일 비비드 조회 실패:", error);
      setDailyVividError(
        error instanceof Error ? error.message : "일일 비비드를 불러올 수 없습니다."
      );
      setDailyVivid([]);
    } finally {
      setIsLoadingDailyVivid(false);
    }
  };

  // 월이 변경되면 일일 비비드 자동 조회
  useEffect(() => {
    if (user?.id && month) {
      fetchDailyVivid();
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.details || "월간 비비드 생성에 실패했습니다.");
      }

      setResult({
        success: true,
        message: `월간 비비드가 성공적으로 생성되고 DB에 저장되었습니다! (ID: ${data.data?.id || "N/A"})`,
        data: data.data,
      });
    } catch (error) {
      console.error("월간 비비드 생성 실패:", error);
      setResult({
        success: false,
  ...
  const handleGenerateTrend = async () => {
    ...
    try {
      const response = await fetch("/api/monthly-vivid/test-trend", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      >
        커스텀 날짜 범위로 월간 비비드를 생성할 수 있는 테스트 페이지입니다.
      </p>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Button
                onClick={fetchDailyVivid}
                disabled={isLoadingDailyVivid || !user || !month}
                size="sm"
                variant="outline"
                className="flex items-center gap-2"
              >
                <RefreshCw
                  className={cn("w-4 h-4", isLoadingDailyVivid && "animate-spin")}
                />
                새로고침
              </Button>
            </div>
                  ...
                  >
                    ...
                    {dailyVivid.length === 0 && (
                      <span className="block mt-2 text-sm" style={{ color: COLORS.status.error }}>
                        ⚠️ 일일 비비드가 없으면 월간 비비드를 생성할 수 없습니다.
                      </span>
                    )}
                  </p>
                </div>
                      ...
                      >
                        ...
                        {feedback.report && (
                          <details className="mt-2">
                            ...
                            >
                              {typeof feedback.report === "string"
                                ? feedback.report
                                : JSON.stringify(feedback.report, null, 2)}
                            </div>
                          </details>
                        )}
                      </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              ⚠️ Trend만 생성 버튼은 테스트용입니다. 기존 월간 비비드의 report가 있어야 동작합니다.
            </p>
          </div>
          ...
          >
            ...
            >
              ...
              실제 월간 비비드는 월의 마지막 일이 지난 후에만 생성 가능합니다.
              여기서는 월을 지정하여 해당 월의 모든 일일 비비드로 월간 비비드를 생성할 수 있습니다.
            </p>
          </div>
        </div>
      )}
