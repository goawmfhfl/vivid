...
import {
  useMonthlyVivid,
  useMonthlyVividById,
} from "@/hooks/useMonthlyVivid";
import { useSubscription } from "@/hooks/useSubscription";
import { MonthlyVividReport } from "./MonthlyVividReport";
import { LoadingSpinner } from "../ui/LoadingSpinner";
import { ErrorDisplay } from "../ui/ErrorDisplay";

type MonthlyVividViewProps = {
  month?: string;
  id?: string;
  onBack: () => void;
};

export function MonthlyVividView({
  month,
  id,
  onBack,
}: MonthlyVividViewProps) {
  // id 유효성 검사
  const isValidId =
  ...
  } = useMonthlyVivid(month || null);
  const {
    data: dataById,
    isLoading: isLoadingById,
    error: errorById,
    refetch: refetchById,
  } = useMonthlyVividById(isValidId ? id : null);
