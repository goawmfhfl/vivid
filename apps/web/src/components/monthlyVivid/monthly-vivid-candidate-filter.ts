 ...
 * 후보 데이터:
 * [
 *   { month: "2025-11", daily_vivid_count: 5 }
 *     → 11월 마지막 일: 2025-11-30
 *     → 2025-11-30 > 2025-11-15 (아직 안 지남)
 *     → ❌ 제외 (아직 마지막 일이 안 지남)
 *
 *   { month: "2025-10", daily_vivid_count: 10 }
 *     → 10월 마지막 일: 2025-10-31
 *     → 2025-10-31 <= 2025-11-15 (이미 지남)
 *     → ✅ 포함
 *
 *   { month: "2025-09", daily_vivid_count: 0 }
 *     → daily_vivid_count: 0 < 1 ❌
 *     → ❌ 제외 (daily_vivid가 없음)
 * ]
 *
 ...
 * - daily_vivid_count >= 1이면 ✅ 포함됨
 *
 * 🔄 생성 후 동작:
 * 1. 사용자가 "생성하기" 버튼 클릭
 * 2. monthly_vivid 테이블에 데이터 저장됨 (is_ai_generated: true)
 * 3. 쿼리 무효화로 새로운 데이터 가져옴
 * 4. API에서 is_ai_generated가 true인 월은 제외됨
 * 5. 결과: 해당 월이 후보 목록에서 사라짐 ✅
 *
 * @param candidates 전체 후보 목록 (API에서 이미 daily_vivid 조건과 is_ai_generated 조건이 적용됨)
...
export function filterMonthlyCandidatesForCreation(
  ...
  for (const candidate of candidates) {
    // 조건 1: daily_vivid가 1개 이상인지 확인 (API에서 이미 필터링되지만 안전장치)
    if (
      candidate.daily_vivid_count === undefined ||
      candidate.daily_vivid_count < 1
    ) {
      continue;
    }
